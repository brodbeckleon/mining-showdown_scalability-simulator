...
export default function JoinPage() {
  ...
  useEffect(() => {
    supabase()
      ...
      .then(({ data }) => {
        ...
        try { parsed = JSON.parse(stored); } catch { sessionStorage.removeItem(sessionKey); return; }
        supabase()
          .from("teams")
          .select("*")
          .eq("id", parsed.teamId)
          .single()
          .then(({ data: tm }) => {
            if (!tm) { sessionStorage.removeItem(sessionKey); return; }
  ...
  const register = useCallback(async () => {
    ...
    try {
      ...
      lastTickRef.current = Date.now();
      sessionStorage.setItem(sessionKey, JSON.stringify({ teamId: data.id, teamName: trimmed, color: teamColor }));
      setStage("playing");
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (liveDeployed && dt > 0 && dt < 10) {
        ...
        const netPerSec = liveMetrics.throughput * CONSTANTS.EARN_RATE - liveMetrics.cost * CONSTANTS.SPEND_RATE;
  ...
  // ─── Derived ──────────────────────────────────────────────────────────
  const metrics = useMemo(() => computeMetrics(cfg, game.load), [cfg, game.load]);
  const bankrupt = wallet <= 0;
  const earnRate = metrics.throughput * CONSTANTS.EARN_RATE - metrics.cost * CONSTANTS.SPEND_RATE;
  ...
  const timeLeft = game.started_at ? gameDuration - computeElapsed(game.started_at, game.running, now) : null;
  ...
  const update = (patch: Partial<TeamConfig>) => setCfg((prev) => ({ ...prev, ...patch }));
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6">
        <div className="text-center">
          <div className="text-4xl font-bold font-jb text-zinc-300 dark:text-zinc-700 mb-3">{code}</div>
          <div className="text-sm text-zinc-500 font-jb mb-6">
            Session not found. Check the code or link and try again.
          </div>
          <Link href="/" className="text-xs text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 font-jb underline">
            Back to home
          </Link>
        </div>
      </main>
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          <Link href="/" className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 font-jb mb-6">
          ...
          <p className="text-sm text-zinc-500 mb-8 font-jb">{t.team.joinSubtitle}</p>

          <div className="space-y-4">
            <div>
              <label htmlFor="team-name" className="text-[11px] uppercase tracking-wider text-zinc-500 dark:text-zinc-400 font-jb">
                {t.team.teamName}
              </label>
              <input
                id="team-name"
              ...
              {teamName.trim() && (
                <div className="mt-2 flex items-center gap-2 text-xs font-jb">
                  <div className="w-3 h-3" style={{ background: colorForName(teamName.trim()) }} />
            ...
            <div className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-3 text-xs text-zinc-500 dark:text-zinc-400 font-jb leading-relaxed">
              <span className="text-emerald-500 dark:text-emerald-400">Wallet:</span>{" "}
    ...
    <main className="min-h-screen p-3 md:p-5">
      {gameOver && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/70 dark:bg-zinc-950/80 backdrop-blur-sm p-6">
          <div className="w-full max-w-sm border-2 bg-white dark:bg-zinc-950 p-8 text-center" style={{ borderColor: color }}>
            <Trophy size={40} className="mx-auto mb-4 text-amber-400" />
            <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-jb mb-1">{t.team.gameOverTitle}</div>
            <div className="text-sm font-semibold mb-5">{teamName}</div>
            <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-jb mb-1">{t.team.finalPlacement}</div>
            <div className="text-6xl font-jb tabular-nums mb-1" style={{ color }}>
              #{myRank > 0 ? myRank : "—"}
            </div>
            ...
            <div className="border-t border-zinc-200 dark:border-zinc-800 pt-4">
              <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-jb mb-1">{t.team.finalScore}</div>
              <div className="text-2xl font-jb tabular-nums" style={{ color }}>{fmt(score)}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto">
            ...
            <div>
              ...
              <div className="text-[10px] text-zinc-500 font-jb">
                {game.running ? (
                  ...
                  <span className="text-zinc-400 dark:text-zinc-600">○ {t.team.waitingHost}</span>
                )}{" "}
                · {t.team.load}: {game.load} req/s ·{" "}
                <span className="text-amber-500 dark:text-amber-400">{code}</span>
              </div>
            </div>
          </div>
          {timeLeft !== null &&
            (() => {
              ...
              return (
                <div className={`text-sm font-jb tabular-nums ${tl <= 0 ? "text-zinc-400 dark:text-zinc-600" : tl < 60 ? "text-red-500 dark:text-red-400" : tl < 120 ? "text-amber-500 dark:text-amber-400" : "text-zinc-500 dark:text-zinc-400"}`}>
        ...
        <div className={`mb-4 border px-3 py-2 flex items-center gap-2 text-xs font-jb ${statusClass}`}>
          {bankrupt ? <AlertTriangle size={13} /> : <CheckCircle2 size={13} />}
          <span className="flex-1">{statusText}</span>
          {myRank > 0 && (
            <span className="text-zinc-500">{t.team.rank} #{myRank}/{allTeams.length}</span>
          )}
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-4">
          ...
          <div className={`lg:col-span-4 space-y-3 ${gameOver ? "pointer-events-none opacity-40" : ""}`}>
            <section className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-[11px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-jb">CHF Budget</h2>
                </div>
                <span className={`text-sm font-jb tabular-nums ${bankrupt || wallet < CONSTANTS.TEAM_BUDGET * 0.1 ? "text-red-500 dark:text-red-400" : wallet < CONSTANTS.TEAM_BUDGET * 0.3 ? "text-amber-500 dark:text-amber-400" : "text-emerald-500 dark:text-emerald-400"}`}>
                  {wallet.toFixed(1)} CHF
                </span>
              </div>
              <Bar percent={(wallet / CONSTANTS.TEAM_BUDGET) * 100} color={bankrupt || wallet < CONSTANTS.TEAM_BUDGET * 0.1 ? "bg-red-500" : wallet < CONSTANTS.TEAM_BUDGET * 0.3 ? "bg-amber-500" : "bg-emerald-500"} />
              {game.running && !bankrupt && (
                <div className={`text-[10px] font-jb tabular-nums ${earnRate >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"}`}>
                  {earnRate >= 0 ? "+" : ""}{earnRate.toFixed(3)} CHF/s · Infra {metrics.cost.toFixed(0)} CHF/h
                </div>
              )}
            </section>
            ...
            <section className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-4 space-y-3">
              <div className="flex items-center gap-2">
                <TrendingUp size={13} className="text-zinc-500 dark:text-zinc-400" />
                <h2 className="text-[11px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-jb">{t.team.verticalSection}</h2>
              </div>
              <Slider label={t.team.cpuCores} value={cfg.cpuPerNode} min={1} max={16} onChange={(v) => update({ cpuPerNode: v })} unit="cores" />
              <Slider label="RAM" value={cfg.ramPerNode} min={1} max={64} onChange={(v) => update({ ramPerNode: v })} unit="GB" />
            </section>

            <section className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-4 space-y-3">
              <div className="flex items-center gap-2">
                <Network size={13} className="text-zinc-500 dark:text-zinc-400" />
                <h2 className="text-[11px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-jb">{t.team.horizontal}</h2>
              </div>
              <Slider label={t.team.nodeCount} value={cfg.nodeCount} min={1} max={6} onChange={(v) => update({ nodeCount: v })} unit="nodes" />
              <Toggle label="Load Balancer" value={cfg.loadBalancer} onChange={(v) => update({ loadBalancer: v })} hint={t.team.lbHint} />
            </section>

            <section className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-4 space-y-3">
              <div className="flex items-center gap-2">
                <Database size={13} className="text-zinc-500 dark:text-zinc-400" />
                <h2 className="text-[11px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-jb">{t.team.sharding}</h2>
              </div>
              <Slider label={t.team.dbShards} value={cfg.shards} min={1} max={6} onChange={(v) => update({ shards: v })} unit="shards" />
            </section>
          </div>
          ...
          <div className="lg:col-span-5 space-y-3">
            <section className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-[11px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-jb">{t.team.topology}</h2>
                </div>
                <div className="flex items-center gap-3 text-[9px] font-jb text-zinc-500">
                  {[["bg-emerald-500", "ok"], ["bg-amber-500", "warn"], ["bg-red-500", "krit"], ["bg-zinc-400 dark:bg-zinc-600", "idle"]].map(([c, l]) => (
                    <span key={l} className="flex items-center gap-1"><span className={`w-1.5 h-1.5 ${c}`} />{l}</span>
                  ))}
                </div>
              </div>
            ...
            {bankrupt ? (
              <section className="border-2 border-red-500 bg-red-500/10 p-5">
                <div className="text-[10px] uppercase tracking-widest text-red-500 dark:text-red-400 font-jb mb-1">BANKRUPT</div>
                <div className="text-xl font-jb text-red-600 dark:text-red-300">{t.team.infraOffline}</div>
                <div className="text-xs text-zinc-500 font-jb mt-2">
                  {t.team.finalScore}: <span className="text-zinc-700 dark:text-zinc-300 tabular-nums">{fmt(score)}</span>
                </div>
              </section>
            ) : (
              <section className="border-2 p-5" style={{ borderColor: color }}>
                <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-jb mb-1">Coins Mined</div>
                <div className="text-4xl font-jb tabular-nums" style={{ color }}>{fmt(score)}</div>
                <div className="text-xs text-zinc-500 font-jb mt-1">
                  {t.team.throughput}:{" "}
                  <span className="text-zinc-700 dark:text-zinc-300">{Math.round(metrics.throughput)}</span> req/s
                  {metrics.dropped > 1 && (
                    <span className="text-red-500 dark:text-red-400 ml-2">−{Math.round(metrics.dropped)} drops</span>
                  )}
                </div>
              </section>
            )}
          ...
          <div className="lg:col-span-3 space-y-3">
            <section className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-4 space-y-3">
              <h3 className="text-[11px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-jb">{t.team.utilization}</h3>
              {[
                { icon: <Cpu size={11} />, label: "CPU", pct: metrics.cpuPercent, color: "bg-emerald-500" },
                { icon: <MemoryStick size={11} />, label: "RAM", pct: metrics.ramPercent, color: "bg-cyan-500" },
                { icon: <Database size={11} />, label: "DB", pct: metrics.dbUtil * 100, color: "bg-violet-500" },
              ].map(({ icon, label, pct, color }) => (
                <div key={label} className="space-y-1">
                  <div className="flex justify-between text-xs font-jb">
                    <span className="flex items-center gap-1.5 text-zinc-500 dark:text-zinc-400">{icon}{label}</span>
              ...
              <div className="pt-2 border-t border-zinc-200 dark:border-zinc-800 grid grid-cols-2 gap-2 text-xs font-jb">
                <div>
                  <div className="text-zinc-500 text-[10px] uppercase tracking-wider">{t.team.responseTime}</div>
                  <div className="tabular-nums text-zinc-700 dark:text-zinc-200">{Math.round(metrics.responseTime)} ms</div>
                </div>
                <div>
                  <div className="text-zinc-500 text-[10px] uppercase tracking-wider">{t.team.appCap}</div>
                  <div className="tabular-nums text-zinc-700 dark:text-zinc-200">{Math.round(metrics.cpuCapacity)} req/s</div>
                </div>
              </div>
            </section>
            ...
            <section className="border border-zinc-200 dark:border-zinc-800 bg-zinc-100/50 dark:bg-zinc-900/30 p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-[11px] uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-jb">Top 5</h3>
              </div>
              <div className="space-y-1.5">
                {sortedTeams.slice(0, 5).map((tm, i) => (
                  <div key={tm.id} className={`flex items-center gap-2 text-xs font-jb ${tm.id === teamId ? "text-emerald-600 dark:text-emerald-400" : "text-zinc-700 dark:text-zinc-300"}`}>
                ...
                {sortedTeams.length === 0 && (
                  <div className="text-[11px] text-zinc-400 dark:text-zinc-600 font-jb">{t.team.noTeamsYet}</div>
                )}
              </div>
            </section>
          </div>
