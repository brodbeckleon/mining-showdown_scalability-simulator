// ─────────────────────────────────────────────────────────────────────────────
// Translations — DE / EN
// ─────────────────────────────────────────────────────────────────────────────

export type Lang = "de" | "en";

export const translations = {
  de: {
    common: {
      back: "zurück",
      leave: "verlassen",
      confirm: "Bestätigen",
      cancel: "Abbrechen",
    },
    home: {
      tagline: "Scalability Multiplayer",
      subtitle:
        "Wer baut die effizienteste Mining-Infrastruktur unter Last? Jedes Team startet mit dem gleichen Wallet. Wer am klügsten skaliert — vertikal, horizontal oder mit Sharding — gewinnt.",
      soloButton: "Solo spielen",
      soloDesc: "Spiele gegen KI-Bots. Wähle den Schwierigkeitsgrad und übertreffe sie.",
      hostButton: "Spiel hosten",
      hostDesc: "Erstelle eine private Session mit einem einzigartigen Join-Link und QR-Code.",
      beamerDesc: "Leaderboard für die Projektion. Live-Ranking aller Teams.",
      teamTitle: "Team beitreten",
      teamDesc:
        "Mitspielen vom Laptop oder Handy. Topologie, Sliders, Live-Score.",
      joinDesc: "Tritt einer bestehenden Session mit Code bei.",
      howToPlay: "Spielablauf",
      step1: "Host erstellt eine Session und teilt den Join-Link.",
      step2:
        "Teams öffnen die Team-Ansicht auf ihren Geräten und treten mit Namen bei.",
      step3: "Host startet das Spiel und passt die Last über die Zeit an.",
      step4: (budget: number) =>
        `Teams starten mit ${budget} CHF Wallet. Infrastrukturkosten laufen laufend ab. Wer bankrott geht, ist raus.`,
      step5:
        "Coins = kumuliert geminte Coins (Throughput × Mining-Rate). Drops = weniger Coins.",
      joinButton: "Session beitreten",
    },
    joinPage: {
      title: "Session beitreten",
      subtitle: "Gib den Code ein, den du vom Host erhalten hast.",
      codeLabel: "Session Code",
      codePlaceholder: "z.B. ABC123",
      joinButton: "Beitreten →",
    },
    beamer: {
      live: "LIVE — MINING IN PROGRESS",
      waiting: "WAITING TO START",
      globalLoad: "Globale Last",
      startWallet: "Start-Budget",
      strategies: "Strategien",
      waitingTeams: "Warte auf Teams...",
      waitingHint: "Join-Link öffnen, Namen eingeben.",
      currentLeader: "aktueller Leader",
      liveStatus: "live",
      pausedStatus: "pausiert",
      scoreFooter: "coins = kumuliert geminte coins · budget in chf",
      labFooter: "Mining Showdown",
      teams: (n: number) => `${n} team${n !== 1 ? "s" : ""}`,
      coinsMined: "geminte coins",
      finalResults: "Endresultate",
      ended: "beendet",
      timerRunning: "läuft",
    },
    host: {
      locked: "Zugang gesperrt",
      lockedHint: "Passwort eingeben um fortzufahren.",
      password: "Passwort",
      wrongPassword: "Falsches Passwort.",
      login: "Einloggen",
      gameControl: "Spielleitung",
      connected: (n: number) => `${n} Team${n !== 1 ? "s" : ""} verbunden`,
      running: "läuft",
      paused: "pausiert",
      gameStatus: "Spielstatus",
      globalLoad: "Globale Last (alle Teams)",
      loadHint:
        "Ändere die Last während des Spiels — Teams müssen darauf reagieren.",
      connectedTeams: "Verbundene Teams",
      noTeams: "Noch niemand beigetreten. Teile den Join-Link.",
      deleteTeamTitle: "Team löschen",
      confirmReset: "Alle Teams und Spielstand löschen?",
      confirmDeleteTeam: (name: string) => `Team "${name}" löschen?`,
      resume: "Fortsetzen",
      start: "Start",
      pause: "Pause",
      reset: "Reset",
      gameDuration: "Spielzeit",
      fluctuateOn: "Fluctuate AN",
      fluctuateOff: "Fluctuate AUS",
    },
    team: {
      joinTitle: "Team beitreten",
      joinSubtitle: "Wähle einen Namen — er erscheint auf dem Leaderboard.",
      teamName: "Team-Name",
      namePlaceholder: "z.B. Hash Rangers",
      colorHint: "Farbe wird automatisch zugewiesen",
      joinButton: "Spiel beitreten →",
      walletHint: (budget: number) =>
        `Startet mit ${budget} CHF Budget. Infrastruktur kostet CHF (laufend). Geminte Coins = Score. CHF ≤ 0 = Game Over.`,
      errorFallback: "Unbekannter Fehler",
      errorCreate: "Konnte Team nicht erstellen",
      waitingHost: "warten auf Host",
      load: "Last",
      rank: "Rang",
      verticalSection: "Vertical (pro Node)",
      cpuCores: "CPU-Kerne",
      horizontal: "Horizontal",
      nodeCount: "Anzahl Nodes",
      lbHint: "Ohne LB arbeitet nur Node 1.",
      sharding: "Sharding",
      dbShards: "DB-Shards",
      topology: "Topologie",
      utilization: "Auslastung",
      responseTime: "Antwortzeit",
      appCap: "App-Cap",
      noTeamsYet: "Noch keine Teams aktiv.",
      statusBankrupt: "BANKRUPT — Infrastruktur offline. Endstand eingefroren.",
      statusNotStarted:
        "Spiel noch nicht gestartet — du kannst aber schon planen.",
      statusPaused: "Spiel pausiert — warte auf den Host.",
      statusDraining: (rate: string) =>
        `CHF Budget schrumpft (${rate}/s) — Infrastruktur zu teuer für aktuellen Durchsatz.`,
      statusBottleneck: (n: number) => `Engpass: ${n} req/s gehen verloren.`,
      statusOk: "System läuft sauber. Mining aktiv.",
      infraOffline: "Infrastruktur offline",
      finalScore: "Endstand",
      throughput: "Throughput",
      gameOverTitle: "Spiel beendet!",
      finalPlacement: "Endplatzierung",
      of: "von",
      participants: "Teilnehmer",
      lockedHint: "Die Konfiguration ist gesperrt.",
      coinsMined: "geminte coins",
    },
    strategyPanel: {
      title: "Skalierungsstrategien",
      subtitle: "aus Bondi & Software Engineering at Google",
      liveClassified: (n: number) =>
        `live klassifiziert · ${n} Team${n !== 1 ? "s" : ""}`,
      more: (n: number) => `+ ${n} mehr`,
      footer:
        "Klassifikation per Heuristik auf Team-Konfiguration. Sharding hat Vorrang vor Combined, Combined vor Load-Balanced, Misconfig (Nodes ohne LB) wird als eigene Kategorie geführt.",
    },
    session: {
      loading: "Session laden…",
      joining: "Session beitreten…",
      notFound: "Session nicht gefunden. Code ungültig oder abgelaufen.",
      joinNotFound: "Session nicht gefunden. Code oder Link prüfen.",
      backToHome: "Zurück zur Startseite",
      shareTitle: "Beitritts-Link",
      shareSubtitle: "Teile diesen Link mit deinen Mitspielern",
      sessionCode: "Session Code",
      clickToCopy: "Klicken zum Kopieren",
      copied: "Kopiert!",
      copyLink: "Link kopieren",
      viewOnly: "Nur ansehen — du bist nicht der Session-Eigentümer.",
    },
  },
  en: {
    common: {
      back: "back",
      leave: "leave",
      confirm: "Confirm",
      cancel: "Cancel",
    },
    home: {
      tagline: "Scalability Multiplayer",
      subtitle:
        "Who builds the most efficient mining infrastructure under load? Every team starts with the same wallet. Who scales smartest — vertically, horizontally, or with sharding — wins.",
      soloButton: "Play Solo",
      soloDesc: "Race against AI bots. Pick your difficulty and outscale them.",
      hostButton: "Host a Game",
      hostDesc: "Create a private session with a unique join link and QR code.",
      beamerDesc: "Leaderboard for projection. Live ranking of all teams.",
      teamTitle: "Join Team",
      teamDesc: "Play from laptop or phone. Topology, sliders, live score.",
      joinDesc: "Join an existing session with a code.",
      howToPlay: "How to Play",
      step1: "Host creates a session and shares the join link.",
      step2: "Teams open the join link on their devices and enter a name.",
      step3: "Host starts the game and adjusts the load over time.",
      step4: (budget: number) =>
        `Teams start with a ${budget} CHF wallet. Infrastructure costs drain it continuously. Go bankrupt and you're out.`,
      step5:
        "Coins = cumulative mined coins (throughput × mining rate). Drops = fewer coins.",
      joinButton: "Join Session",
    },
    joinPage: {
      title: "Join Session",
      subtitle: "Enter the code you received from the host.",
      codeLabel: "Session Code",
      codePlaceholder: "e.g. ABC123",
      joinButton: "Join →",
    },
    beamer: {
      live: "LIVE — MINING IN PROGRESS",
      waiting: "WAITING TO START",
      globalLoad: "Global Load",
      startWallet: "Start Budget",
      strategies: "Strategies",
      waitingTeams: "Waiting for teams...",
      waitingHint: "Open the join link, enter a name.",
      currentLeader: "current leader",
      liveStatus: "live",
      pausedStatus: "paused",
      scoreFooter: "coins = cumulative mined coins · budget in chf",
      labFooter: "Mining Showdown",
      teams: (n: number) => `${n} team${n !== 1 ? "s" : ""}`,
      coinsMined: "coins mined",
      finalResults: "Final Results",
      ended: "ended",
      timerRunning: "running",
    },
    host: {
      locked: "Access Restricted",
      lockedHint: "Enter password to continue.",
      password: "Password",
      wrongPassword: "Wrong password.",
      login: "Login",
      gameControl: "Game Control",
      connected: (n: number) => `${n} team${n !== 1 ? "s" : ""} connected`,
      running: "running",
      paused: "paused",
      gameStatus: "Game Status",
      globalLoad: "Global Load (all teams)",
      loadHint: "Change the load during the game — teams must adapt.",
      connectedTeams: "Connected Teams",
      noTeams: "Nobody joined yet. Share the join link.",
      deleteTeamTitle: "Delete team",
      confirmReset: "Delete all teams and game state?",
      confirmDeleteTeam: (name: string) => `Delete team "${name}"?`,
      resume: "Resume",
      start: "Start",
      pause: "Pause",
      reset: "Reset",
      gameDuration: "Duration",
      fluctuateOn: "Fluctuate ON",
      fluctuateOff: "Fluctuate OFF",
    },
    team: {
      joinTitle: "Join Team",
      joinSubtitle: "Choose a name — it appears on the leaderboard.",
      teamName: "Team Name",
      namePlaceholder: "e.g. Hash Rangers",
      colorHint: "Color is assigned automatically",
      joinButton: "Join game →",
      walletHint: (budget: number) =>
        `Starts with ${budget} CHF budget. Infrastructure costs CHF (continuously). Mined coins = score. CHF ≤ 0 = Game Over.`,
      errorFallback: "Unknown error",
      errorCreate: "Could not create team",
      waitingHost: "waiting for host",
      load: "Load",
      rank: "Rank",
      verticalSection: "Vertical (per Node)",
      cpuCores: "CPU Cores",
      horizontal: "Horizontal",
      nodeCount: "Node Count",
      lbHint: "Without LB only Node 1 is active.",
      sharding: "Sharding",
      dbShards: "DB Shards",
      topology: "Topology",
      utilization: "Utilization",
      responseTime: "Response Time",
      appCap: "App Cap",
      noTeamsYet: "No teams active yet.",
      statusBankrupt: "BANKRUPT — Infrastructure offline. Final score frozen.",
      statusNotStarted: "Game not started yet — you can plan ahead.",
      statusPaused: "Game paused — waiting for the host.",
      statusDraining: (rate: string) =>
        `CHF budget draining (${rate}/s) — infrastructure too expensive for current throughput.`,
      statusBottleneck: (n: number) => `Bottleneck: ${n} req/s being dropped.`,
      statusOk: "System running clean. Mining active.",
      infraOffline: "Infrastructure offline",
      finalScore: "Final Score",
      throughput: "Throughput",
      gameOverTitle: "Game Over!",
      finalPlacement: "Final Placement",
      of: "of",
      participants: "Participants",
      lockedHint: "Configuration is locked.",
      coinsMined: "coins mined",
    },
    strategyPanel: {
      title: "Scaling Strategies",
      subtitle: "from Bondi & Software Engineering at Google",
      liveClassified: (n: number) =>
        `live classified · ${n} team${n !== 1 ? "s" : ""}`,
      more: (n: number) => `+ ${n} more`,
      footer:
        "Classification by heuristic on team configuration. Sharding takes precedence over Combined, Combined over Load-Balanced, Misconfig (nodes without LB) is its own category.",
    },
    session: {
      loading: "Loading session…",
      joining: "Joining session…",
      notFound: "Session not found. The code may be invalid or expired.",
      joinNotFound: "Session not found. Check the code or link and try again.",
      backToHome: "Back to home",
      shareTitle: "Join Link",
      shareSubtitle: "Share this with your players",
      sessionCode: "Session Code",
      clickToCopy: "Click to copy",
      copied: "Copied!",
      copyLink: "Copy Link",
      viewOnly: "View only — you are not the session owner.",
    },
  },
};

export type Translations = typeof translations.de;
